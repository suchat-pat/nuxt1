        ...
        <v-row>
            <v-col cols="12">
                <v-form v-if="user.status_eva === 2 || user.status_eva === 3">
                    ...
                    <div class="text-end"><v-btn @click="report" color="blue" class="d-print-none">พิมพ์</v-btn></div>
                    ...
                    <v-row v-for="(topic,index1) in topics" :key="topic.id_topic" class="mt-2">
                        <v-col cols="12">
                            ...
                            <v-table class="table">
                                <tr>
                                    <th class="border pa-2 bg-grey text-black" style="width: 10%;">ตัวชี้วัด</th>
                                    <th class="border pa-2 bg-grey text-black" style="width: 10%;">รายละเอียด</th>
                                    <th class="border pa-2 bg-grey text-black" style="width: 10%;">น้ำหนักคะแนน</th>
                                    <th class="border pa-2 bg-grey text-black" style="width: 10%;">คะแนนเต็ม</th>
                                    <th class="border pa-2 bg-grey text-black" style="width: 10%;">คะแนนที่ได้</th>
                                </tr>
                    ...
                    <div class="mt-4">
                        <v-card class="pa-2">
                            ...
                            <v-row v-for="(commit,index3) in commits" :key="commit.id_commit" class="mt-2">
                                <v-col cols="12">{{ index3+1 }}.{{ commit.level_commit }} : {{ commit.detail_commit || 'รอการประเมิน' }}</v-col>
                            </v-row>
                        </v-card>
                    </div>
                    <div class="mt-4">
                        <v-row class="mt-2 text-center">
                            <v-col cols="4" v-for="signatures in commits" :key="signatures.id_commit"><img :src="`http://localhost:3001/uploads/signature/${signatures.signature}`" style="width: 20%;"> <br> {{ signatures.first_name }} {{ signatures.last_name }} <br> {{ signatures.level_commit }}</v-col>
                        </v-row>
                    </div>
                </v-form>
