                    ...
                    <v-table class="table mt-3">
                        <thead>
                            <tr class="bg-gray-400">
                                ...
                                <th class="text-center border">ตรวจสอบคะแนนประเมิน</th>
                                <th class="text-center border">ดำเนินการยืนยันผล</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(items,index) in result" :key="items.id_eva">
                                ...
                                <td class="text-center border">
                                    <v-btn v-if="items.signature" color="green" class="text-white" size="small" @click="add(items.id_eva)">ยืนยันผลแล้ว</v-btn>
...
const add = (id_eva:number) => {
    router.push({path:`/Committee/Signature${id_eva}`})
}
const check = (id_eva:number) => {
    router.push({path:`/Committee/Detail_commit${id_eva}`})
}
